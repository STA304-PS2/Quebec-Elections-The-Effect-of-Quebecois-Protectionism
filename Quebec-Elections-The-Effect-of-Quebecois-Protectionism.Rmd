...
# Results
```{r}
total_votes_by_culture <- Quebec_survey_data %>%  #generate a new data set 
  select(Upcoming_election, Protect_Culture) %>%#select the collumns we wish to use
  mutate(Percentage_voted_for = 0) %>% 
  group_by(Protect_Culture) %>% #group all rows by date
  Percentage_voted_for = count(Upcoming_election) 
 

total_votes_by_culture

tabyl(Quebec_survey_data, Upcoming_election, Protect_Culture) %>%
  adorn_percentages("col") %>%
  adorn_pct_formatting(digits = 1)


Figure1 <- total_votes_by_culture %>% 
